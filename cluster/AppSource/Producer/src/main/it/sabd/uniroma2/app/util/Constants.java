...
public class Constants {
    ...
    //If set to true, prints the Flink Stream ouput
    public static final boolean PRINT_FLINK_OUTPUT = false;

    //If set to true, the dataset gets injected with a proportional delay
    public static final boolean PROPORTIONAL_REPLAY = false;
    //Duration of a Single minute expressed in Microseconds
    public static final float MINUTE_DURATION = 1f;

    //System enviroment variable name which decides if the application is
    // the Kafka Producer/Consumer or the Flink Jar Topology
    ...
    static {
        ...
        WINDOW_MAP.put(WindowSize.ONE_HOUR, new Tuple3<>(Time.hours(1L), Time.hours(1L), Time.seconds(900)));
        WINDOW_MAP.put(WindowSize.TWO_HOUR, new Tuple3<>(Time.hours(2L), Time.hours(2L), Time.seconds(4500)));
    }

    public static final Duration ALLOWED_LATENESS = Duration.ofMinutes(5L);
    ...
    //How many Cells to report in the leaderboard
    public static final Integer QUERY2_TOP = 3;
